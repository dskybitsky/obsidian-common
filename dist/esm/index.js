...
class ReactPlugin extends Plugin {
    constructor(app, manifest) {
        super(app, manifest);
        this.rootsIndex = new Map();
        this.elementsFactoriesIndex = new Map();
    ...
    onDataviewMetadataChange(_type, page) {
        if (!page.path) {
            return;
        }
        this.renderRootsByPath(getRootFolder(page.path));
    }
    registerElement(root, path, elementFactory) {
    ...
    registerRoot(root, path) {
        ...
        ((_a = this.rootsIndex.get(path)) !== null && _a !== void 0 ? _a : []).push(root);
    ...
    renderRootsByPath(path) {
        if (!this.rootsIndex.has(path)) {
            return;
        }
        const roots = this.rootsIndex.get(path);
        for (const root of (roots !== null && roots !== void 0 ? roots : [])) {
...
class Reader {
    ...
    getPage(path) {
        var _a;
        return ((_a = this.api.page(`${path}/${Reader.FolderIndex}`)) !== null && _a !== void 0 ? _a : this.api.page(path));
    }
    getPages(query) {
    ...
    getPagesAtDepth(query, depth = 0) {
        return this.getPages(query).filter((page) => Reader.isPageAtDepth(page, depth));
    }
    getPagesByPathAtDepth(path, depth = 0) {
        return this.getPagesAtDepth(Reader.pathToSource(path), depth);
    }
    getPagesByPathAtDepthRel(path, depth = 0) {
        const pathDepth = path.split('/').length;
        return this.getPagesByPathAtDepth(path, pathDepth + depth);
    }
    static isPageAtDepth(page, depth) {
        const pageDepth = page.file.path.split('/').length;
        return page.file.name === Reader.FolderIndex
            ? pageDepth === depth + 1
            : pageDepth === depth;
    }
    static pathToSource(path) {
...
Reader.FolderIndexExt = `${Reader.FolderIndex}.md`;
...
class Writer {
    ...
    async createPage(path, metadata, content) {
...
export { Container, InternalLink, Message, ReactPlugin, Reader, ToolBar, Check as ToolBarCheck, Edit as ToolBarEdit, Writer, getFolder, getRootFolder, setActiveTabTitle };
